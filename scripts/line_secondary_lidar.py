...
offset_lines_pub = rospy.Publisher('offset_new_line0', LineSegmentList, queue_size=1)
offset_lines_marker_pub = rospy.Publisher('offset_new_line0_marker', Marker, queue_size=1)


def redis_value_init(init):
    if init:
        red.hset("line", "line0_x_offset", 0.6)
        red.hset("line", "line0_y_offset", 0.0)
        red.hset("line", "line0_remove_angle_0", 0)
        red.hset("line", "line0_remove_angle_1", 90)
...
def offsetLines(lines_msg):
    ...
    line0_remove_angle_0 = int(red.hget("line", "line0_remove_angle_0"))
    line0_remove_angle_1 = int(red.hget("line", "line0_remove_angle_1"))
    ...
    for line in lines_msg.line_segments:
        x_dir_offset = float(red.hget("line", "line0_x_offset"))
        y_dir_offset = float(red.hget("line", "line0_y_offset"))  
        ...
        if remove_line == 1:
            if line0_remove_angle_0 - offset < degree <  line0_remove_angle_0 + offset:
                if len(idx_list) < 1:
                    idx_list.append(idx)
                else:
                    idx_list.append(idx)                  
            elif line0_remove_angle_1 - offset < degree <  line0_remove_angle_1 + offset:
                if len(idx_list) < 1:
                    idx_list.append(idx)
                else:
                    idx_list.append(idx)              
...
def listener():
    rospy.init_node('sub_line_node_0', anonymous=True)
    sub_lines = rospy.Subscriber('/line0/line_segments', LineSegmentList, offsetLines, queue_size=1)
    rospy.spin()
