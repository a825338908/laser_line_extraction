...
def init_redis(r=None, restore_default=False):
    ...
    if restore_default:
        ...
        r.set("slider_left_distance", 0)
        r.set("slider_right_distance",0)
    # high: 1560 < x < 2630
    # mid: 890 < x < 2630
    # low: 1020 < x < 1910
    return r
...
class SliderPaint():
    ...
    def slider_action(self, slider_device, action = SliderAction.STOP):
        '''
    ...
    def move_to(self, target,slider_device):
        if slider_device == "right":
            ...
            if not self.is_processing_command:  # first time; initiate action
                ...
                self.slider_action(slider_device, SliderAction.NONE)
            ...
            if step.strip('') == '':
                ...
                if command[0:7] == 'blind_x': #blind_x3000
                    ...
                    if tmp[1] == 'slider':
                        if tmp[2] == 'move_to':
                            target = float(tmp[3])
                            slider_device = str(tmp[4])
                            self.set_slider_move_distance(target,slider_device) 
                            try:
                                ms = float(tmp[4])
                            except:
                            ...
                            if self.step_t0 is None:
                                ...
                                if remaining > 0:
                                    ...
                                    step_keep_running = self.slider.move_to(target,slider_device)  # true if arrived target/min/end
                                else:
                    ...
                    if check_angle- merge_deg < angle_deg < check_angle + merge_deg:
                        ...
                        if move_x_dir:
                            if  (start_point_y > 0.0 and end_point_y < 0.0):
                                ...
                                found = False
                                break                           
                            elif (-offset_close_origin<start_point_y<offset_close_origin or -offset_close_origin<end_point_y<offset_close_origin):
                                angles.pop(i)
                                distances.pop(i)
                                found = False
                                break
                            ...
                            if (start_point_x > 0.0 and end_point_x < 0.0):
                                ...
                                found = False
                                break
                            elif (-offset_close_origin<start_point_x<offset_close_origin or -offset_close_origin<end_point_x<offset_close_origin):
                                angles.pop(i)
                                distances.pop(i)
                                found = False
                                break   
